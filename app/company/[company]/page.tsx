...
import { BurnSiteTabs } from "@/components/company/BurnSiteTabs";
...
export function generateMetadata({ params }: { params: { company: string } }): Metadata {
  ...
  return { title: company ? company.name : "Burn Site" };
}

export default function BurnSitePage({ params }: { params: { company: string } }) {
  const company = getCompany(params.company);
  if (!company) notFound();
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-16 md:px-8">
      {company.hallOfFlame && <InductionMoment companySlug={company.slug} companyName={company.name} />}
      <div className="relative border-b border-void-line pb-10">
        <BlastMark
          size={140}
          intensity={heatIntensity(company.burnRatio)}
          seed={company.slug}
        ...
        <div className="flex items-center gap-2">
          ...
          {company.hallOfFlame && <HazardFlower size={20} className="text-blowtorch" title="Hall of Flame" />}
        </div>
        ...
        <div className="mt-6 flex flex-wrap items-end gap-6">
          <HeatCounter value={company.burnRatio} size="lg" simulateLive label="Burn Ratio" className="text-blowtorch" />
          <TierBadge tier={company.tier} />
        </div>

        <p className="label mt-6 text-[10px] text-paper-faint">
          Burn Site live since {formatDateLong(company.firstBlowupAt)}
        </p>
      </div>
      ...
      <section className="border-b border-void-line py-10">
        <BurnSiteTabs blowups={blowups} flashpoints={companyFlashpoints} pledgeCount={company.pledgeCount} />
      </section>
